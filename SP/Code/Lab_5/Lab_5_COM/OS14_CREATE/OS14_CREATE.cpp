...
int main(int argc, char* argv[]) {
	if (argc != 7) {
		std::cerr << "Usage: ./OS13_CREATE.exe <Capacity> <SecSnapshotInterval> <MaxKeyLength> <MaxPayloadLength> <UserGroup> <FileName>" << std::endl;
		return EXIT_FAILURE;
	}
	...
	try {
		...
		HT::HTHANDLE* handle = OS14_LIB::OS14_HTCOM::Create_HT(h, atoi(argv[1]), atoi(argv[2]), atoi(argv[3]), atoi(argv[4]), argv[5], argv[6]);
		if (handle) {
			...
			if (OS14_LIB::OS14_HTCOM::Close_HT(h, handle)) {
