...
extern "C" __declspec(dllexport) STDAPI DllGetClassObject(REFCLSID rclsid, REFIID riid, LPVOID* ppv) {
    ...
    *ppv = nullptr;
    ...
    if (!IsEqualCLSID(rclsid, CLSID_OS14)) {
...
extern "C" __declspec(dllexport) STDAPI DllRegisterServer() {
    ...
    HRESULT hr = StringFromCLSID(CLSID_OS14, &clsidString);
    if (FAILED(hr)) {
        return hr;
    }
    ...
    if (SUCCEEDED(hr)) {
        hr = SetRegKeyValue(HKEY_CLASSES_ROOT, keyPath, NULL, L"OS14 COM Object");
    }
    if (FAILED(hr)) {
    ...
    hr = SetRegKeyValue(HKEY_CLASSES_ROOT, L"OS14_COM.1", NULL, L"OS14 COM Object");
    if (FAILED(hr)) {
        CoTaskMemFree(clsidString);
        return hr;
    }

    hr = SetRegKeyValue(HKEY_CLASSES_ROOT, L"OS14_COM.1\\CLSID", NULL, clsidString);
    if (FAILED(hr)) {
        CoTaskMemFree(clsidString);
        return hr;
    }
...
extern "C" __declspec(dllexport) STDAPI DllUnregisterServer() {
    ...
    hr = StringFromCLSID(CLSID_OS14, &clsidString);
    if (FAILED(hr)) {
    ...
    if (RegDeleteTreeW(HKEY_CLASSES_ROOT, L"OS14_COM.1") != ERROR_SUCCESS) {
        hr = S_FALSE;
    }

#else

    RegDeleteKeyW(HKEY_CLASSES_ROOT, L"OS14_COM.1\\CLSID");
    RegDeleteKeyW(HKEY_CLASSES_ROOT, L"OS14_COM.1");
